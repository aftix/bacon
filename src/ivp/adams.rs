...
pub trait AdamsSolver {
    ...
    fn update_dt(
        &mut self,
        _error: <Self::Complex as ComplexField>::RealField,
    ) -> Result<u8, String> {
        Ok(1)
    }
...
pub fn adams<S: AdamsSolver, T: Clone>(
    mut solver: S,
    (t_initial, t_final): (
        <S::Complex as ComplexField>::RealField,
        <S::Complex as ComplexField>::RealField,
    ),
    y_0: &[S::Complex],
    ...
    let rk = super::RungeKutta::<S::Complex>::default()
        .with_dt(solver.dt())?
        .build()?;
    let initial = super::runge_kutta(
        ...
        (
            t_initial,
            t_initial
                + S::Complex::from(S::predictor_coefficients().len() as f64).real() * solver.dt(),
        ),
        y_0,
        derivs,
        params,
    ...
    for (i, state) in considering.iter().enumerate() {
        if i == 0 {
            ...
            memory.push_back(derivs(
                state.0,
                considering[i - 1].1.column(0).as_slice(),
                &mut params_considering,
            ));
        }
    }
    ...
    'out: loop {
        ...
        for i in 0..considering.len() {
            predictor += &memory[considering.len() - i - 1]
                * S::Complex::from_real(S::predictor_coefficients()[i] * solver.dt());
        }

        let implicit = derivs(
            time + solver.dt(),
            predictor.column(0).as_slice(),
            &mut params_considering,
        );
        ...
        if S::predictor_corrector() {
            ...
            for i in 0..considering.len() {
                corrector += &memory[considering.len() - i - 1]
                    * S::Complex::from_real(S::corrector_coefficients()[i + 1] * solver.dt());
            }
            corrector +=
                &implicit * S::Complex::from_real(S::corrector_coefficients()[0] * solver.dt());
            ...
            if result > 0 {
                ...
                if result == 2 || time + dt_old > t_final {
                    ...
                    *params = params_considering.clone();
                    ...
                    let initial = super::runge_kutta(
                        ...
                        (
                            time,
                            time + S::Complex::from(S::predictor_coefficients().len() as f64)
                                .real()
                                * dt_old,
                        ),
                        y_0,
                ...
                *params = params_considering.clone();
                ...
                let initial = super::runge_kutta(
                    ...
                    (
                        time,
                        time + S::Complex::from(S::predictor_coefficients().len() as f64).real()
                            * solver.dt(),
                    ),
                    y_0,
                    derivs,
...
impl<N: ComplexField + From<f64> + Copy> AdamsBashforthBuilder<N> {
    ...
    pub fn with_dt(
        &mut self,
        dt: <N as ComplexField>::RealField,
    ) -> Result<&mut AdamsBashforthBuilder<N>, String> {
...
impl<N: ComplexField + From<f64> + Copy> PredictorCorrectorBuilder<N> {
    ...
    pub fn with_dt_min(
        &mut self,
        dt_min: <N as ComplexField>::RealField,
    ) -> Result<&mut PredictorCorrectorBuilder<N>, String> {
